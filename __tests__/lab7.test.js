describe('Basic user flow for Website', () => {
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for (let i = 1; i < prodItems.length; i++) {
      const shadowRoot = prodItems[i].shadowRoot;
    ...
    const cartCount = await cartCountTextProp.jsonValue();
  ...
  await page.reload();
  const allButtonsRemembered = await page.$$eval('product-item', (prodItems) => {
  ...
  it('Checking the localStorage to make sure cart is correct', async () => {
    ...
    expect(cartData).toBe(expectedCart);
  });
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    const allButtonsReset = await page.$$eval('product-item', (prodItems) => {
      for (let i = 0; i < prodItems.length; i++) {
        const shadowRoot = prodItems[i].shadowRoot;
        const button = shadowRoot.querySelector('button');
